// PureC-OS AR9285 (ath9k-class) bring-up driver — TEST branch, phase 1.
//
// Что умеет эта фаза:
//  - находит PCI 168C:002B, включает MEM+BM, маппит BAR0;
//  - читает SREV, проверяет AR9285 (macVersion 0xC0);
//  - будит MAC (AR_MAC_SLEEP), гасит прерывания;
//  - читает STA_ID0/STA_ID1 как MAC;
//  - регистрирует net_device "wlan0" + wifi_ops в wifi-подсистеме.
//
// Чего НЕ умеет (phase 2): EEPROM/калибровка, BB/RF init,
// QCU/DCU + TX/RX дескрипторы, scan MLME, assoc, WPA. Поэтому
// transmit/poll — заглушки, is_connected() всегда false, scan
// сразу сообщает пустой результат чтобы не клинить wifi_poll().
//
// Регистровые смещения сверены с Linux drivers/net/wireless/ath/ath9k/reg.h
// и drivers/net/wireless/ath/reg.h (ISC-лицензия, Atheros Communications).
// Логика init написана с нуля под PureC-OS, без копипасты GPL-кода.
